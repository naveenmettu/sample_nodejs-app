...
const IdentityProvider = function (options) {
  const self = this;
  ...
  self.options = {
    ...
    getPostURL: function (audience, authnRequestDom, req, callback) {
      if (self.serviceProviders[audience]) {
        let url = self.serviceProviders[audience].recipient;
        if (req.authnRequest && req.authnRequest.acsUrl && req.authnRequest.acsUrl !== self.serviceProviders[audience].recipient) {
          if (self.serviceProviders[audience].acceptedAcsUrls
            && self.serviceProviders[audience].acceptedAcsUrls.includes(req.authnRequest.acsUrl)) {
  ...
  self.serviceProviders = options.serviceProviders;
...
IdentityProvider.prototype.parseSignInRequest = function (req, callback) {
  ...
  samlp.parseRequest(req, function(err, data) {
    ...
    const spOptions = self.serviceProviders[options.issuer];
...
IdentityProvider.prototype.signIn = function (options) {
  ...
  authOptions.allowRequestAcsUrl = this.serviceProviders[options.issuer].allowRequestAcsUrl;
  authOptions.signingCert = this.serviceProviders[options.issuer].signingCert;
  authOptions.digestAlgorithm = this.serviceProviders[options.issuer].digestAlgorithm;
