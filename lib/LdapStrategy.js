...
class LdapStrategy extends Strategy {
   ...
   */
  search(client, username, callback) {
    const options = {
    ...
    logger.debug(
      `Searching for user with options ${JSON.stringify(options)} from ${this.options.searchBase}`
    )
  ...
  _authenticate(username, password, callback) {
    ...
    return client.bind(this.options.serviceAccountDn, this.options.serviceAccountPassword, err => {
      if (err) {
        logger.error(
          `Unable to bind LDAP client for ${this.options.serviceAccountDn} to ${this.options.url}`
        )
        return callback(err)
      }

      return this.search(client, username, (err, user) => {
        if (err) {
          return callback(err)
        }
  ...
  authenticate(req, options) {
    ...
    const username =
      (req.body ? req.body.username : null) || (req.query ? req.query.username : null)
    const password =
      (req.body ? req.body.password : null) || (req.query ? req.query.password : null)

    logger.debug(
      `Authenticating request; ${req.user ? '' : 'no '}user present; username ${username ||
        'not present'}`
    )
    ...
    this._authenticate(username, password, (err, user) => {
      if (err) {
      ...
      if (this.verify) {
        return this.verify(user, (err, verifiedUser) => {
          if (err) {
            logger.warn(
              `Failed during user profile verification for user:\n${user}\nError:\n${err}`
            )
            ...
            return this.error(
              new Error(
                `Did not receive user object from profile verification function for user:\n${user}`
              )
            )
          }
